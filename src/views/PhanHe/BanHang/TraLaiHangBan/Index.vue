...
export default {
  ...
  data() {
    return {
      fields: this.lodash.getTableFields(Documents.chungtu.tralaihangban),
      filter: {
        loai: {
          value: null,
          keyFilter: "id",
  ...
  computed: {
    ...mapGetters({
      list: "chungtu.tralaihangban.list",
      detailPaneSize: "chungtu.tralaihangban.panesize",
  ...
  methods: {
    showDetail(obj) {
      this.$store.commit("chungtu.tralaihangban.panesize", 50);
      this.$store.dispatch("chungtu.tralaihangban.select", obj.id);
    },
    onPaneResized(panes) {
      this.$store.commit("chungtu.tralaihangban.panesize", panes[1].size);
    },
  },
};
</script>