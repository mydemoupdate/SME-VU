...
export default {
  name: "TabChiTietBanHang",
  computed: {
    ...mapGetters({
      obj: "chungtu.banhang.selected",
  ...
  methods: {
    create() {
      this.$store.dispatch("chungtu.banhang.create", {
        ...this.obj,
        hachtoantienhang: [],
        hachtoanchieckhau: [],
        hachtoanthuegtgt: [],
    ...
    save() {
      this.$store.dispatch("chungtu.banhang.update", this.obj);
    },
    destroy() {
      this.$store.dispatch("chungtu.banhang.delete");
    },
  },
};
</script>
