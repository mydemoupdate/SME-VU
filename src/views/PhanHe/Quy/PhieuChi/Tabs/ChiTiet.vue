...
export default {
  name: "TabChiTietPhieuChi",
  computed: {
    ...mapGetters({
      obj: "chungtu.phieuchi.selected",
  ...
  methods: {
    create() {
      this.$store.dispatch("chungtu.phieuchi.create", {
        ...this.obj,
        hachtoan: [],
      });
    },
    save() {
      this.$store.dispatch("chungtu.phieuchi.update", this.obj);
    },
    destroy() {
      this.$store.dispatch("chungtu.phieuchi.delete");
    },
  },
};
</script>
