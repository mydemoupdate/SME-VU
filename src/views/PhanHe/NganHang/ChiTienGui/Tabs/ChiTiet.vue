  ...
  <CRow class="mx-1">
    <CCol col="8">
      <CForm class="size-sm">
        ...
        <CRow>
          <CCol sm="12" size="sm" class="form-group">
            <label class="col-form-label-sm">Tài khoản chi</label>
            <Treeselect
              :appendToBody="true"
...
export default {
  name: "TabChiTietChiTienGui",
  computed: {
    ...mapGetters({
      obj: "chungtu.chitiengui.selected",
  ...
  methods: {
    create() {
      this.$store.dispatch("chungtu.chitiengui.create", {
        ...this.obj,
        hachtoan: [],
      });
    },
    save() {
      this.$store.dispatch("chungtu.chitiengui.update", this.obj);
    },
    destroy() {
      this.$store.dispatch("chungtu.chitiengui.delete");
    },
  },
};
</script>
