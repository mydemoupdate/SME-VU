  ...
  <CRow class="mx-1">
    <CCol col="8">
      <CForm class="size-sm">
        ...
        <CInput size="sm" label="Người nhận" :value.sync="obj.nguoinhan" />
        <CRow>
          <CCol sm="12" size="sm" class="form-group">
...
export default {
  ...
  methods: {
    create() {
      this.$store.dispatch("chungtu.xuatkho.create", {
        ...this.obj,
      });
    },
    save() {
      this.$store.dispatch("chungtu.xuatkho.update", this.obj);
    },
    destroy() {
      this.$store.dispatch("chungtu.xuatkho.delete");
    },
  },
};
</script>
