...
export default {
  ...
  data() {
    return {
      fields: this.lodash.getTableFields(Documents.chungtu.giamgiahangmua),
      filter: {
        loai: {
          value: null,
          keyFilter: "id",
  ...
  computed: {
    ...mapGetters({
      list: "chungtu.giamgiahangmua.list",
      detailPaneSize: "chungtu.giamgiahangmua.panesize",
  ...
  methods: {
    showDetail(obj) {
      this.$store.commit("chungtu.giamgiahangmua.panesize", 50);
      this.$store.dispatch("chungtu.giamgiahangmua.select", obj.id);
    },
    onPaneResized(panes) {
      this.$store.commit("chungtu.giamgiahangmua.panesize", panes[1].size);
    },
  },
};
</script>