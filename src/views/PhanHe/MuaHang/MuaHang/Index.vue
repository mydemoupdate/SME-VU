...
export default {
  ...
  data() {
    return {
      fields: this.lodash.getTableFields(Documents.chungtu.muahang),
      filter: {
        loai: {
          value: null,
          keyFilter: "id",
  ...
  computed: {
    ...mapGetters({
      list: "chungtu.muahang.list",
      detailPaneSize: "chungtu.muahang.panesize",
  ...
  methods: {
    showDetail(obj) {
      this.$store.commit("chungtu.muahang.panesize", 50);
      this.$store.dispatch("chungtu.muahang.select", obj.id);
    },
    onPaneResized(panes) {
      this.$store.commit("chungtu.muahang.panesize", panes[1].size);
    },
  },
};
</script>